...
namespace AngryNinjas
{
	public class TheLevel : CCLayer
	{
		...
		{
			CCLog.Log("Show Menu");
			Director.PushScene( TheMenu.GetScene(Window));
		}
		...
		public static CCScene GetScene(CCWindow mainWindow)
		{
			...
			CCLayer layer = new TheLevel(mainWindow.WindowSizeInPixels);
		...
		void MoveScreen(int amountToShiftScreen)
		{
			...
			if (areWeOnTheIPad)
			{ //the label seems to stay in one place, but really the entire layer is moving (only do this on iPad)
		...
		public void TouchesMoved(System.Collections.Generic.List<CCTouch> touches, CCEvent touchEvent)
		{
		...
		void AdjustBackStrap(float angle)
		{
			...
			else if (angle < 300)
			{
				...
				strapBack.RotationX = strapFront.RotationX * 1.0f;
				strapBack.RotationY = strapFront.RotationY * 1.0f;
		...
		{
			get { return new CCTransitionFadeDown(1, TheLevel.GetScene(Window)); }
		}

		CCTransitionScene Transition1
		{
			get { return new CCTransitionFlipX(1, TheLevel.GetScene(Window), CCTransitionOrientation.RightOver); }
		}

		CCTransitionScene Transition2
		{
			get { return new CCTransitionFade(1, TheLevel.GetScene(Window)); }
		}

		CCTransitionScene Transition3
		{
			get { return new CCTransitionFlipAngular(1, TheLevel.GetScene(Window), CCTransitionOrientation.DownOver); }
		}

		CCTransitionScene Transition4
		{
			get { return new CCTransitionFadeTR(1, TheLevel.GetScene(Window)); }
		}
