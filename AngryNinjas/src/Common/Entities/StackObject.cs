		...
		{ 
			...
			{  //if we included frames to show for breaking and the current frame is less than the max number of frames to play
				...
				if (currentFrame < 10) {
					...
					sprite.Texture = new CCSprite(String.Format("{0}_000{1}", baseImageName, currentFrame)).Texture;

				} else if (currentFrame < 100) { 
					
					sprite.Texture = new CCSprite(String.Format("{0}_00{1}", baseImageName, currentFrame)).Texture;
